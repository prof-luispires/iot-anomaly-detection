...
def get_temperature_ylim(df: pd.DataFrame,
                         observed_col: str = "temp_observed",
                         clean_col: str = "temp_clean") -> Tuple[float, float]:
    """
    Define scientifically coherent limits for the temperature Y-axis,
    differentiating synthetic, field-like and real-world datasets.
    """
    sid = str(df["scenario_id"].iloc[0])

    vals = []
    if observed_col in df.columns:
        vals.append(pd.to_numeric(df[observed_col], errors="coerce").to_numpy())
    if clean_col in df.columns:
        vals.append(pd.to_numeric(df[clean_col], errors="coerce").to_numpy())
    ...
    if sid.startswith("REAL_"):
        ...
        ymin = max(-5.0, p1 - pad)
        ymax = min(45.0, p99 + pad)

        if ymax <= ymin:
            ymin, ymax = float(np.min(y)) - 1.0, float(np.max(y)) + 1.0
...
def plot_reconstruction_comparison(df: pd.DataFrame,
    ...
    ax.plot(x, df["temp_observed"], label="Observed", linewidth=1.2, alpha=0.85)
    ...
    for method_name in methods_to_show:
        if method_name in method_outputs:
            ax.plot(
                x,
                method_outputs[method_name]["cleaned"],
                linewidth=1.0,
                label=f"{method_name} cleaned"
            )
    ...
    ymin, ymax = get_temperature_ylim(df)
...
def process_dataset(df: pd.DataFrame,
    ...
    if sid == "SF":
        plot_reconstruction_comparison(
            df=df,
            method_outputs=method_outputs,
            save_path=dataset_result_dir / f"{dataset_name}_field_like_reconstruction.png"
        )
    ...
    if sid.startswith("REAL_"):
        plot_reconstruction_comparison(
            df=df,
            method_outputs=method_outputs,
            save_path=dataset_result_dir / f"{dataset_name}_real_world_reconstruction.png"
        )
